...
const ProductSingle = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5050/product?slug=${slug}`).then((res) => {
      setProducts(res.data[0]);
    });

    axios
      .get(`http://localhost:5050/category/${products.catid}/product`)
      .then((res) => {
        setRelated(res.data);
      });
  }, [products]);

  const { name, dec, photo, reg_price, sale_price, rating } = products;
